...
the features, retrains the model, asks Claude for the summary, and
saves everything to a dated report file instead of just printing it.

Run: python src/day07_pipeline.py
Output: outputs/reports/YYYY-MM-DD.md (plus the same files day01/day03
already write to data/ and data/features/, refreshed with today's data).
"""
...
def run_model_and_llm() -> tuple[dict, str]:
    """Day 4/5/6 step: retrain per ticker, predict, and ask Claude to summarize."""
    reports = {}
...
def write_report(reports: dict, summary: str) -> str:
    ...
    lines.append("| Ticker | Close | RSI14 | Predicted | Precision | Recall | Strategy | Buy & Hold |")
    lines.append("|---|---|---|---|---|---|---|---|")
...
def main():
    ...
    print("Step 3/3: retraining models and generating the LLM summary...")
